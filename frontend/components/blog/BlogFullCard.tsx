import { Box, Flex, Heading, HStack, Link, Text, useBreakpointValue } from '@chakra-ui/react'
  ...
  return (
    <Flex mt={2} direction={{ base: 'column', sm: 'row' }} justifyContent="space-between">
      ...
      <Flex flex={1.75} direction="column" justifyContent="center" mt={{ base: 3, md: 0 }} pl={{ base: 0, sm: 3 }}>
        <Heading as={Link} fontSize="4xl" fontWeight="600" lineHeight="tight" to={'#'}>
          {'TODO'}
        </Heading>
        ...
        <Flex mt={2} justifyContent="space-between">
          <BlogAuthor hasLongDate={useBreakpointValue({ base: false, md: true })} name={'TODO'} date={new Date()} />
          <HStack fontWeight="light" fontSize={{ base: 'sm', md: 'xs', lg: 'sm' }} textColor={useColorModeValue('gray.600', 'gray.400')}>
            <FaClock />
            <Text>{readTimeInMinutes(100)}&nbsp;perc</Text>
          </HStack>
        </Flex>
